<template>
  <div class="pane pane-sm sidebar">
    ...
    <div class="show_table" v-if="!run_mode">
      <div class="sidebar-section">
...
<script>
  ...
  export default {
    ...
    watch: {
      selected_database: 'requestTablesAndNav',
      filterText(val) {
    ...
    methods: {
      ...
      clickNode(node) {
        if (node.id === 1)
          this.request({
            channel: 'get-platform-chart-request',
            payload: node
          })
        // else if (node.id === 3){
        //   this.request({
        //     channel: 'get-copo-chart-request',
        //     payload: node
        //   })
        // }
      },
